...
cdParams.fsigma             = 157.5;  %photoelastic stress coefficient = wave_length / (C*h)
...
dsParams.algorithm              = 'levenberg-marquardt'; %'levenberg-marquardt' Algorithm to use. Other options: 'trust-region-reflective', 'interior-point'
dsParams.maxIterations          = 500; % maximum iterations allowed for each fit
dsParams.maxFunctionEvaluations = 1000; %maximum number of function evaluations for each fit
dsParams.functionTolerance      = 0.001; % if subsequent fits change by less than this amount, stop
dsParams.scaling                = 0.1; %scale image by this much
dsParams.maskradius             = 0.8; %percent of disk to use for fit (cuts outer edge)
